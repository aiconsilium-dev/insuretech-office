...
export default function ClaimsPage() {
  ...
  return (
    <div>
      <div className="text-[22px] font-extrabold mb-4">접수 관리</div>
        ...
        {(["all", "A", "B", "C"] as const).map((f) => (
          <button
            ...
            className={clsx(
              ...
              typeFilter === f ? "!bg-black !text-white !border-black" : "!bg-white !text-gray-600 !border-gray-200"
            )}
          >
            {f === "all" ? "전체" : `TYPE ${f}`}
          </button>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-3 scrollbar-hide">
        {(["all", "resident", "office"] as const).map((f) => (
          <button
            ...
            className={clsx(
              "btn btn-sm !rounded-[var(--radius-pill)] !border whitespace-nowrap",
              sourceFilter === f ? "!bg-black !text-white !border-black" : "!bg-white !text-gray-600 !border-gray-200"
            )}
          >
...
function ClaimCard({
  ...
  return (
    <Card variant="outlined" className="!p-4 mb-3 shadow-sm" onClick={onToggle}>
      <div className="flex justify-between items-start">
        <div className="flex-1 min-w-0">
          <div className="text-xs text-gray-400 mb-1">{claim.id}</div>
          <div className="text-[15px] font-bold">{loc}</div>
      ...
      {isOpen && (
        <div className="pt-4 mt-3 border-t border-gray-100" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex gap-2 mt-1">
            ...
            <button className="btn btn-outline btn-sm flex-1 !rounded-[var(--radius-pill)] !text-black !border-gray-200" onClick={onCycleStatus}>상태 변경</button>
          </div>
...
function ClaimDetail({ claim }: { claim: Claim }) {
  ...
  if (claim.typeClass === "A") {
    return (
      <>
        <div className="bg-gray-50 rounded-[var(--radius-card)] p-4 mb-3 border border-gray-200 border-l-[3px] border-l-black">
          <div className="text-sm font-bold mb-1.5">시공사 하자 — 하자보수 청구 대상</div>
          <div className="text-[13px] text-gray-600 leading-relaxed">{d.defectDetail || d.desc}</div>
        </div>
  ...
  if (claim.typeClass === "B") {
    return (
      <>
        <div className="bg-gray-50 rounded-[var(--radius-card)] p-4 mb-3 border border-gray-200 border-l-[3px] border-l-gray-400">
          <div className="text-sm font-bold mb-1.5">면책 대상</div>
          <div className="text-[13px] text-gray-600 leading-relaxed">{d.exemptReason}</div>
        </div>
  ...
  return (
    <>
      <div className="bg-gray-50 rounded-[var(--radius-card)] p-4 mb-3 border border-gray-200 border-l-[3px] border-l-primary">
        <div className="text-sm font-bold mb-1.5">보험금 산출</div>
        <div className="text-[13px] text-gray-600 leading-relaxed">
          AI 적산 금액: <strong>{d.aiAmount || "-"}</strong>
        </div>
      </div>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="mb-3">
      ...
      <div className="text-sm text-black leading-relaxed">{value}</div>
    </div>
  );
}
